...
class PostRepositoryImpl(
    ...
    private fun boardIdEq(boardId: Long): BooleanExpression =
        post.get("board", Board::class.java).getNumber("id", java.lang.Long::class.java).eq(boardId)

    private fun eqCategoryId(categoryId: Long?): BooleanExpression? =
        categoryId?.let { post.get("category", Category::class.java).getNumber("id", java.lang.Long::class.java).eq(it) }
